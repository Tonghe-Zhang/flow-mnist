...
class ShortCutReFlow(nn.Module):
    ...
    def loss_flow(self, xt, t, cls, v):
        if self.use_fixed_dt_input==True:
            dt_base = torch.tensor(np.log2(self.max_denoising_steps)).repeat(self.batch_size).to(self.device)
            # print(f"[flow]: xt.shape = {xt.shape}, t.shape = {t.shape}, dt_base.shape = {dt_base.shape}, cls.shape={cls.shape}")
        else:
    ...
    def loss_bs(self, xt_bs, t_bs, dt_base, cls, v_bs):
        ...
        # print(f"[bs]: xt.shape = {xt_bs.shape}, t.shape = {t_bs.shape}, dt_base.shape = {dt_base.shape}, cls.shape={cls.shape}")
        ...
        for epoch in tqdm(range(self.start_epoch, self.end_epoch+ 1, 1)):
            ...
            with tqdm(total=len(train_loader), desc=f'Train epoch {self.epoch}/step: ') as pbar:
                for step, (x, cls) in enumerate(train_loader):
                    ...
                    (xt, t), v = self.generate_target_flow(x, cls)
                    loss_flow = self.loss_flow(xt, t, cls, v)
                    
                    (xt_bs, t_bs, dt_base), v_bs = self.generate_bs_target(x, cls)
                    loss_bs = self.loss_bs(xt_bs, t_bs, dt_base, cls, v_bs)
