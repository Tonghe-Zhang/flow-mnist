...
class UNetShortCutModel(nn.Module):
    ...
    def __init__(
        ...
        #############################################################################################
        # modification
        self.dt_embed =nn.Embedding(num_embeddings=int(np.log2(max_denoising_steps)), embedding_dim=time_embed_dim)
        #############################################################################################
    ...
    def forward(self, x, t, dt, y=None):
        ...
        #####################################################################################
        t_emb = self.time_embed(timestep_embedding(timesteps, self.model_channels))   # modification//2
        
        dt_emb= self.dt_embed(dt_steps, self.model_channels)# modification//2

        emb=torch.cat((t_emb,dt_emb), dim=-1)# cat
        #####################################################################################
        if self.num_classes is not None:
            assert y.shape == (x.shape[0],)
            emb = torch.cat((emb, self.label_emb(y)), dim=-1)
        #####################################################################################
        h = x.type(self.dtype)
...
NUM_CLASSES = 1000
class UNetShortCutModelWrapper(UNetShortCutModel):
    def __init__(
        self,
        dim,
        num_channels,
        ...
        return super().__init__(
            ...
            max_denoising_steps = max_denoising_steps,  # modification
            use_checkpoint=use_checkpoint,
            use_fp16=use_fp16,
            num_heads=num_heads,
            num_head_channels=num_head_channels,
    ...
    def forward(self, x, t, dt_base, y=None, *args, **kwargs):
        return super().forward(x, t, dt_base, y=y)